...
format_storage_devices() {
    ...
    for DEVICE in $DEVICELIST; do
        ...
        # Skip devices smaller than 64GB (e.g., empty card readers, SD cards, small USB drives)
        DEVICE_SIZE=$(blockdev --getsize64 "$DEVICE" 2>/dev/null || echo "0")
        if [ "$DEVICE_SIZE" -lt 68719476736 ]; then
            echo "Skipping $DEVICE: device too small (${DEVICE_SIZE} bytes, minimum 64GB)" 2>&1 | sudo tee -a $FULA_LOG_PATH
            continue
        fi
