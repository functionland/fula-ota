...
if echo "$response" | grep -q 'HTTP/.* 200 OK'; then
    ...
    if [ -n "${CLUSTER_CRDT_TRUSTEDPEERS}" ]; then
        ...
        exec ipfs-cluster-service daemon --upgrade --bootstrap "/dnsaddr/ipfs-cluster.${CLUSTER_CRDT_TRUSTEDPEERS}.functionyard.fula.network/p2p/${CLUSTER_CRDT_TRUSTEDPEERS}" --leave
    else
