...
PINNING_SERVICE_ENDPOINT=http://127.0.0.1:6000
FULA_HOST=127.0.0.1
FULA_PORT=9000
...
# The service validates `token_hash = sha256(presented)` (postgres_service.go:804;
# NULL expiry is allowed). session_token is NOT NULL UNIQUE — fill it with the
# hash too (it's never matched for hashed sessions).
psqlc "INSERT INTO sessions (username, session_token, token_hash, expires_at) VALUES ('$U', '$JWT_HASH', '$JWT_HASH', NOW() + interval '2 hours') ON CONFLICT DO NOTHING" >/dev/null 2>&1 || true
psqlc "UPDATE sessions SET token_hash = '$JWT_HASH', expires_at = NOW() + interval '2 hours' WHERE session_token = '$JWT_HASH'" >/dev/null 2>&1 || true
